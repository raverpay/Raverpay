...
async function main() {
  ...
  try {
    ...
    console.log(
      '   Sending withdrawal request WITHOUT Idempotency-Key header...',
    );
    ...
    try {
      ...
      console.log(
        '   ❌ FAILED: Request succeeded but should have been blocked!',
      );
    } catch (error: any) {
    ...
    console.log(
      '   Sending 3 rapid withdrawal preview requests (Limit: 1 per 5s)...',
    );
    ...
    const successCount = results.filter(
      (r) => r === 'Success' || r.startsWith('Error: 400'),
    ).length; // 400 might happen if validation fails

    if (rateLimitedCount > 0) {
      console.log(
        `   ✅ SUCCESS: ${rateLimitedCount} requests were rate limited!`,
      );
    } else {
