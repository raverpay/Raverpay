...
export class SmsService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (this.publicKey && this.secretKey) {
      this.logger.log('✅ VTPass SMS service initialized');
    } else {
      this.logger.warn(
  ...
  async sendVerificationCode(
    ...
    if (!this.enabled) {
      this.logger.log(`📱 [MOCK] Verification code SMS to ${phone}: ${code}`);
      return true;
    }

    if (!this.publicKey || !this.secretKey) {
      this.logger.warn(
        `📱 [MOCK] Would send verification code to ${phone}: ${code}`,
      );
      return true;
    }
    ...
    try {
      ...
      if (response.data.responseCode === 'TG00') {
        ...
        if (message.status === 'SENT' || message.status === 'DND_SENT') {
          this.logger.log(
            `✅ Verification SMS sent to ${phone} (ID: ${message.messageId}, Network: ${message.network})`,
          );
          return true;
        } else {
          this.logger.error(
            `Failed to send SMS to ${phone}: ${message.status} - ${message.description}`,
          );
          return false;
        }
      } else {
        this.logger.error(
          `VTPass SMS API error: ${response.data.responseCode} - ${response.data.response}`,
        );
        return false;
      }
    } catch (error) {
  ...
  async sendPasswordResetSms(
    ...
    if (!this.enabled) {
      this.logger.log(`📱 [MOCK] Password reset SMS to ${phone}: ${code}`);
      return true;
    }

    if (!this.publicKey || !this.secretKey) {
      this.logger.warn(
        `📱 [MOCK] Would send password reset code to ${phone}: ${code}`,
      );
      return true;
    }
    ...
    try {
      ...
      if (response.data.responseCode === 'TG00') {
        ...
        if (msg.status === 'SENT' || msg.status === 'DND_SENT') {
          this.logger.log(
            `✅ Password reset SMS sent to ${phone} (ID: ${msg.messageId})`,
          );
          return true;
        }
      }
      ...
      this.logger.error(
        `Failed to send password reset SMS: ${response.data.response}`,
      );
      return false;
    } catch (error) {
  ...
  async sendTransactionAlert(
    ...
    if (!this.enabled) {
      this.logger.log(`📱 [MOCK] Transaction alert SMS to ${phone}`);
      return true;
    }

    if (!this.publicKey || !this.secretKey) {
      this.logger.warn(`📱 [MOCK] Would send transaction alert to ${phone}`);
      return true;
    }
    ...
    try {
      ...
      if (response.data.responseCode === 'TG00') {
        ...
        if (msg.status === 'SENT' || msg.status === 'DND_SENT') {
          this.logger.log(
            `✅ Transaction alert SMS sent to ${phone} (ID: ${msg.messageId})`,
          );
          return true;
        }
      }
      ...
      this.logger.error(
        `Error sending transaction alert SMS to ${phone}:`,
        error,
      );
      return false;
    }
  ...
  async checkBalance(): Promise<number> {
    if (!this.publicKey || !this.secretKey) {
      this.logger.warn('📱 [MOCK] SMS balance check - no credentials');
      return 0;
    }
    ...
    try {
      ...
      this.logger.log(`💰 SMS Balance: ${balance} units`);
      return balance;
    } catch (error) {
      this.logger.error('Error checking SMS balance:', error);
      return 0;
    }
  }
