...
import { Transaction, PaginatedResponse, TransactionStatistics } from '@/types'
...
export const transactionsApi = {
  ...
  getStatistics: async (params?: Record<string, any>): Promise<TransactionStatistics> => {
  ...
  getPending: async (params?: Record<string, any>): Promise<PaginatedResponse<Transaction>> => {
  ...
  getFailed: async (params?: Record<string, any>): Promise<PaginatedResponse<Transaction>> => {
    const response = await apiClient.get<PaginatedResponse<Transaction>>(
  ...
  reverse: async (transactionId: string, reason: string): Promise<any> => {
    const response = await apiClient.post(`/admin/transactions/${transactionId}/reverse`, {
      reason,
    })
    return response.data
  },
