...
import { GiftCardOrder, PaginatedResponse, GiftCardStatistics } from '@/types'

export const giftcardsApi = {
  getAll: async (params?: Record<string, any>): Promise<PaginatedResponse<GiftCardOrder>> => {
  ...
  getStatistics: async (params?: Record<string, any>): Promise<GiftCardStatistics> => {
    const response = await apiClient.get<GiftCardStatistics>('/admin/giftcards/stats', { params })
    return response.data
  },

  getPending: async (params?: Record<string, any>): Promise<PaginatedResponse<GiftCardOrder>> => {
  ...
  approve: async (orderId: string, amount: number): Promise<any> => {
    const response = await apiClient.post(`/admin/giftcards/${orderId}/approve`, { amount })
    return response.data
  },

  reject: async (orderId: string, reason: string): Promise<any> => {
    const response = await apiClient.post(`/admin/giftcards/${orderId}/reject`, { reason })
    return response.data
  },
