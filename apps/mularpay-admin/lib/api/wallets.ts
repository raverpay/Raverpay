...
import { Wallet, PaginatedResponse, WalletStatistics } from '@/types'

export const walletsApi = {
  getAll: async (params?: Record<string, any>): Promise<PaginatedResponse<Wallet>> => {
    const response = await apiClient.get<PaginatedResponse<Wallet>>('/admin/wallets', { params })
  ...
  adjust: async (
    ...
    const response = await apiClient.post(`/admin/wallets/${userId}/adjust`, {
      amount,
      type,
      reason,
    })
