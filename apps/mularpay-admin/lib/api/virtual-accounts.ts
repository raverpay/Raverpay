...
export const virtualAccountsApi = {
  getAll: async (params?: Record<string, any>): Promise<PaginatedResponse<VirtualAccount>> => {
  ...
  getById: async (accountId: string): Promise<VirtualAccount> => {
    const response = await apiClient.get<VirtualAccount>(`/admin/virtual-accounts/${accountId}`)
    return response.data
  },
